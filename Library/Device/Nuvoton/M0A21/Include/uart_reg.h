...
typedef struct
{
    ...
    * |        |          |Note 1: Refer to Figure 6.11-13 and Figure 6.11-14 for UART function mode.
    * |        |          |Note 2: Refer to Figure 6.11-24 and Figure 6.11-25 for RS-485 function mode.
     ...
     * |        |          |Note: The exit from mute mode condition and each control and interactions of this field are explained in 6.11.5.10 (LIN slave mode).
     ...
     * |        |          |The master sends header event and sends response event and the slave detects header event and receives response event will reset the counter
     ...
     * |        |          |Note 2: This bit is only valid in LIN Slave mode (SLVEN (UART_LINCTL[0]) = 1).
     ...
     * |        |          |This bit is set by hardware when a LIN header reception time-out is detected in LIN slave mode and be cleared by writing 1 to it
     ...
     * |        |          |Note1: If WKRS485EN (UART_WKCTL[3]) is enabled, the RS-485 Address Match (AAD mode) wake-up cause this bit is set to u20181u2019.
     * |        |          |Note2: This bit can be cleared by writing "1" to it.
     * |        |          |Note3: This bit is valid in UART0 and UART1.
     ...
     * @var UART_T::LINRTOUT
     * Offset: 0x4C  UART LIN Response Time-out Register
     * ---------------------------------------------------------------------------------------------------
     * |Bits    |Field     |Descriptions
     * | :----: | :----:   | :---- |
     ...
     * @var UART_T::LINWKCTL
     * Offset: 0x50  UART LIN Wake-up Control Register
     * ---------------------------------------------------------------------------------------------------
     * |Bits    |Field     |Descriptions
     * | :----: | :----:   | :---- |
     * |[23:0]  |LINWKC    |LIN Send Wake-up Signal Length Counter
     ...
     * |        |          |Note 1: When this bit is set, the UART will send LIN wake-up automatically
     ...
     * |        |          |Note 1: When the system is in Power-down mode, LIN wake-up event will wake up system from Power-down mode.
     ...
     * |        |          |Note 1: If LINWKEN (UART_LINWKCTL[28]) is enabled, the LIN wake-up event will cause this bit set to u20181u2019.
    ...
    __IO uint32_t LINRTOUT;              /*!< [0x004C] UART LIN Response TimeOut Register                               */
    __IO uint32_t LINWKCTL;              /*!< [0x0050] UART LIN Wake-up Control Register                                */
    __I  uint32_t RESERVE0[1002];
